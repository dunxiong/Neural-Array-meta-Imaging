...
class FusionNAFnet1(nn.Module):
    def __init__(self, params):
        super(FusionNAFnet1, self).__init__()
        ...
        self.conv_l2_k2 =  NAFBlock(120)

        self.conv_l2_k3 = nn.Conv2d(in_channels=240, out_channels=120, kernel_size=3, padding=1, stride=1, groups=1,bias=True)
        self.conv_l2_k4 = NAFBlock(120)
        ...
        self.conv_l1_k3 = nn.Conv2d(in_channels=120, out_channels=60, kernel_size=3, padding=1, stride=1, groups=1, bias=True)
        self.conv_l1_k4 = NAFBlock(60)
        self.conv_l1_k5 = NAFBlock(60)
        ...
        self.conv_l0_k3 = nn.Conv2d(in_channels=60, out_channels=30, kernel_size=5, padding=2, stride=1, groups=1, bias=True)
        self.conv_l0_k4 = NAFBlock(30)
        self.conv_l0_k5 = NAFBlock(30)
    ...
    def forward(self, wien_decon_4x, wien_decon_2x, wien_decon_1x):
        ...
        conv_l0_k1 = self.conv_l0_k1(conv_l0_k0)
        conv_l0_k2 = self.conv_l0_k2(conv_l0_k1)
        down_l0 = self.down_l0(conv_l0_k2)

        conv_l1_k0 = torch.cat([deconv1, down_l0], dim=1)
        conv_l1_k1 = self.conv_l1_k1(conv_l1_k0)
        conv_l1_k2 = self.conv_l1_k2(conv_l1_k1)
        down_l1 = self.down_l1(conv_l1_k2)

        conv_l2_k0 = torch.cat([deconv2, down_l1], dim=1)
        conv_l2_k1 = self.conv_l2_k1(conv_l2_k0)
        conv_l2_k2 = self.conv_l2_k2(conv_l2_k1)

        conv_l2_k3 = self.conv_l2_k3(torch.cat([conv_l2_k0, conv_l2_k2], dim=1))
        conv_l2_k4 = self.conv_l2_k4(conv_l2_k3)

        up_l2 = self.up_l2(conv_l2_k4)
        conv_l1_k3 = self.conv_l1_k3(torch.cat([conv_l1_k2, up_l2], dim=1))
        conv_l1_k4 = self.conv_l1_k4(conv_l1_k3)
        conv_l1_k5 = self.conv_l1_k5(conv_l1_k4)

        up_l1 = self.up_l1(conv_l1_k5)

        conv_l0_k3 = self.conv_l0_k3(torch.cat([conv_l0_k2, up_l1], dim=1))
        conv_l0_k4 = self.conv_l0_k4(conv_l0_k3)
        conv_l0_k5 = self.conv_l0_k5(conv_l0_k4)

        # output
        out = self.out_conv(conv_l0_k5)
        out = torch.clamp(out, 0.0, 1.0)

        return out

class FusionNAFnet2(nn.Module):
    def __init__(self, params):
        super(FusionNAFnet2, self).__init__()
