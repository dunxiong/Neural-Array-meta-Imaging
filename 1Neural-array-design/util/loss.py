...
def Norm_loss(G_img, gt_img, args):
    ...
    loss = tf.reduce_mean(metric(G_img - gt_img))
...
def P_loss(G_img, gt_img, vgg_model, args):
    ...
    loss = tf.add_n([tf.reduce_mean(metric( (G_layer_out - gt_layer_out) / 255. ))
...
def G_loss(G_img, gt_img, vgg_model, args):
    ...
    MSSSIM = tf.reduce_mean(tf.image.ssim_multiscale(G_img, gt_img, max_val=1.0))

    metrics = {'PSNR':PSNR, 'SSIM':SSIM, 'MSSSIM':MSSSIM}
    ...
    if not args.msssim_loss_weight == 0.0:
        MSSSIM_loss_val = args.msssim_loss_weight * (1 - MSSSIM)

    Content_loss_val = Norm_loss_val + P_loss_val + Spatial_loss_val  + MSSSIM_loss_val
    loss_components = {'Norm':Norm_loss_val, 'P':P_loss_val, 'Spatial':Spatial_loss_val, 'MSSSIM':MSSSIM_loss_val}
